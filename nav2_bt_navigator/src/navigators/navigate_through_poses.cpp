  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  if (!node->has_parameter("waypoint_states_blackboard_id")) {
    node->declare_parameter("waypoint_states_blackboard_id", std::string("waypoint_states"));
  }

  waypoint_states_blackboard_id_ = node->get_parameter("waypoint_states_blackboard_id").as_string(); 
...
NavigateThroughPosesNavigator::onLoop()
{
  ...
  feedback_msg->waypoint_states = 
    blackboard->get<std::vector<nav2_msgs::msg::WaypointState>>(waypoint_states_blackboard_id_);
...
NavigateThroughPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  auto waypoint_states = std::vector<nav2_msgs::msg::WaypointState>(goals_array.goals.size());
  blackboard->set<decltype(waypoint_states)>(waypoint_states_blackboard_id_,
      std::move(waypoint_states));
